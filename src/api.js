...
export const getMyActivity = () => api.get('/players/me/activity');
...
export const updateSelfDetails = (id, data) => api.patch(`/players/${id}/self-details`, data);
...
export const getPlayerNameHistory = (id) => api.get(`/players/${id}/name-history`);
...
export const getPendingGrowDeposits = () => api.get('/grow-deposits/pending');
export const getGrowHistory = (from, to) => {
  const params = {};
  if (from) params.from = from;
  if (to) params.to = to;
  return api.get('/grow-deposits/history', { params });
};
export const confirmGrowDeposit = (id) => api.post(`/grow-deposits/confirm/${id}`);
